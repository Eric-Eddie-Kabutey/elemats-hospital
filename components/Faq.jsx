      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="space-y-4">
          {FAQ_ITEMS.map((item, index) => (
            <div 
              key={index}
              className="group border border-neutral-100 rounded-3xl overflow-hidden shadow-sm transition-all duration-300 hover:shadow-md"
            >
              <button
                onClick={() => setOpenIndex(openIndex === index ? -1 : index)}
                className="w-full px-8 py-6 text-left flex items-center justify-between bg-white hover:bg-zinc-50 transition-colors"
